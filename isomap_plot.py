...
def isomap_plot(embed_network, embed_reg, embed_nois, embed_pert, base_network, img_reg, img_noise, num, three_dim=True, save=False, where=False): 
    ...
    if three_dim:
        ...
        plt.title('Isomap 3D \n Curves: Y10 (orange), Y1 noisy (blue) \n Point color - predicted class: spiral (red), elliptical (blue), merger (black) \n Perturbed images - large points: Y10 image (+), Y1  noisy (*), perturbed image (^)', fontsize=25)

        ax.plot_trisurf(proj[:num, 0], proj[:num, 1], proj[:num, 2], cmap="autumn", alpha = 0.2)
        ax.plot_trisurf(proj[num:num2, 0], proj[num:num2, 1], proj[num:num2, 2],cmap="winter", alpha=0.2)
        ax.scatter(proj[:num, 0], proj[:num, 1], proj[:num, 2], c=color[predict_reg[:]],marker='o', s=20)
        ax.scatter(proj[num:num2, 0], proj[num:num2, 1], proj[num:num2, 2],c=color[predict_noi[:]],marker='o', s=20)

        ax.scatter(proj[num2:num3, 0], proj[num2:num3, 1], proj[num2:num3, 2],c=color[pr_reg[:]],marker='+', s=400)
        ax.scatter(proj[num3:num4, 0], proj[num3:num4, 1], proj[num3:num4, 2],c=color[pr_noi[:]],marker='*', s=400)
        ax.scatter(proj[num4:, 0], proj[num4:, 1], proj[num4:, 2],c=color[pr_pert[:]],marker='^', s=400)
        ...
        ax.scatter(proj[num2:num3, 0], proj[num2:num3, 1],c=color[pr_reg[:]],marker='+', s=400)
        ax.scatter(proj[num3:num4, 0], proj[num3:num4, 1],c=color[pr_noi[:]],marker='*', s=400)
        ax.scatter(proj[num4:, 0], proj[num4:, 1],c=color[pr_pert[:]],marker='^', s=400)
